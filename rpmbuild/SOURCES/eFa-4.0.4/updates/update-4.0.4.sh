...
# Make sure yara rules are DISABLED
...
if [[ -z $(grep razorhome /var/spool/postfix/.razor/razor-agent.conf) ]]; then
    ...
    su -l -c "/bin/cat /usr/share/doc/$samplepath/sample-spam.txt | razor-report -d" -s /bin/bash postfix >/dev/null 2>&1
...
# Install eFa token db and release logic if not present
/usr/bin/mysql -e "use efa;" >/dev/null 2>&1
if [[ $? -ne 0 ]]; then
...
# Bump to 10.4 on CentOS 7
if [[ $centosver -eq 7 ]]; then
  sed -i "/^\[mariadb\-10\.1\]$/ c\[mariadb-10.4\]" /etc/my.cnf.d/mariadb-server.cnf
